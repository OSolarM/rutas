<h1>Impresi&oacute;n Gu&iacute;as Ordenes de Transporte Carabineros</h1>
...
<tr bgcolor="#ebf4fb">
    ...
    <td align=center><strong>{t}Orden de Compra{/t}</strong></td>
...
<tr>
    ...
    <td bgcolor=white align="center">{$rec.orna_orden_compra}</td>
    <!--<td bgcolor=white>{$rec.orna_no_guia}</td>$smarty.foreach.foo.index-->
    <td bgcolor=white><input type="text" id="orna_no_guia{$rec.id}" name="orna_no_guia{$rec.id}" value="{$rec.orna_no_guia_value}" size=10 maxlength=10 style="text-align:right"/></td>
    <td bgcolor=white> 
...
<script language="javascript">
   ...
   function imprimir(n) {
      ...
      document.frm.action="{/literal}{$APP_HTTP}{literal}/ordenes_carabineros/imprimir_guia/"+n+"/"+orna_no_guia.value;
      document.frm.submit();
   }
</script>
{/literal}
