...
<p>Datos b&aacute;sicos de la Expedici&oacute;n</p>
<table><tr><td>
<table>
   ...
   <tr><td align="right">{lbl}Lastre{/lbl}  </td><td>  {select name="expe_lastre"}|,S&iacute;|S,No|N{/select}</td></tr>
   <tr><td align="right">{lbl}Bloqueo{/lbl}    </td><td>{select name="expe_bloqueo"}|,S&iacute;|S,No|N{/select}</td></tr>
   <tr><td colspan="2" style="background:lightgray"><table cellpadding="1" cellspacing="1">
             <tr style="background:#aaabbcc"><td align="center">Gu&iacute;a</td>
             ...
             <tr style="background:white">{hidden name="idRec"}
                 ...
                 {glink caption="Cancelar" action="edit"}/{$id}{/glink}
   ...
   <td align="right">{if $id ne "" && $expe_cerrado ne "S"}{glink caption="Cerrar Expedici&oacute;n" confirm="Seguro cerrar expedici&oacute;n?" action="cerrar"}/{$id}{/glink}{else}Cerrado{/if}<!--<button type="submit">Grabar</button>--></td></tr>
</table>
